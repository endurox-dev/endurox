...
static PSRegFunction _stream_methods[] = {
...
void init_streamclass(HPSCRIPTVM v)
{
	...
	if(PS_FAILED(ps_get(v,-2))) {
		...
		while(_stream_methods[i].name != 0) {
			PSRegFunction &f = _stream_methods[i];
...
PSRESULT declare_stream(HPSCRIPTVM v,const PSChar* name,PSUserPointer typetag,const PSChar* reg_name,PSRegFunction *methods,PSRegFunction *globals)
{
