...
ndrx_main() or _tmstartserver(). If flag is not configured, binary will not start
and message will be printed to ULOG. The error would look like:

--------------------------------------------------------------------------------

05828:20200503:23031209:atmi.sv1    :Error ! Buildserver thread option says single-threaded, but MINDISPATCHTHREADS=5 MAXDISPATCHTHREADS=5
...
|2|cmdid|Const "I"|Mand|1|Record type identifier
