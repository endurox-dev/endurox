...
|4|Cluster node id|N..5|Mand|1|Cluster Node id which started transaction
|5|TMSRV id|N..5|Mand|1|Transaction manager server id (srvid from ndrxconfig.xml)
|6|Transaction timeout|N..19|Mand|1|Transaction timeout set by tpbegin(3)
|7|Initial list of resource managers participating in transaction (active)|ANS..1023|Mand|In version 1 this
...
no real reason for this record. Thus from Enduor/X 6.1, this will have only 
first RM logged which opened the
transaction, basically the same id found in Field Nr. 3.
|=========================================================
...
|Nr|Field name|Type|Mand/Opt|Version|Description
|1|UTC Time stamp|N13|Mand|1|UTC Time stamp when record added
|2|Record type id|Const "S"|Mand|1|Record type identifier
|3|Stage code|N..2|Mand|1|*0* - transaction does no exists,
*5* - transaction is active in processing,
*20* - transaction is aborting,
*25* - transaction aborted with hazard,
*30* - heuristically aborted,
*35* - aborted ok,
*40* - preparing,
*55* - committed (hazard),
...
|Nr|Field name|Type|Mand/Opt|Version|Description
|1|UTC Time stamp|N13|Mand|1|UTC Time stamp when record added
|2|Record type id|Const "R"|Mand|1|Record type identifier - resource manager status
|3|Resource Manager ID|N..5|Mand|1|Particular ID of resource manager for which
status is being tracked
|4|Resource Manager status|A1|Mand|1|0x0 - NULL, 
*n* - Non transaction,
...
|5|Transaction ID|N..19|Opt|2|Transaction id, per resource manager. If value
