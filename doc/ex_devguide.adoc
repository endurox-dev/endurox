...
- 'TP_REQUEST', logging facility code 'R' - this is user logs, set on per thread/request basis. 
The log level is set with keyword 'tp'. Facility is defined with macros 'LOG_FACILITY_TP_REQUEST'.
...
impact if 'TP_REQUEST' log level is different. The request logging can be open 
by *tplogsetreqfile(3)*. Logger can be closed by *tplogclosereqfile(3)*.

- If 'TP_THREAD' is open (debug file set), then all logs including NDRX, UBF 
and TP will log here. Thread logger can be open by doing *tplogconfig(LOG_FACILITY_TP_THREAD, ...)*.
Thread logger can be closed by *tplogclosethread(3)*
...
to grep the big log files (based on each service output) and glue together the picture
...
During the Enduro/X C library works, it is assumed that is not preemptible for 
...
- *c client* - Generate C client application. This make sample C client app which in case
if UBF buffer is select for data buffer, the sample call is made to *TESTSV* XATMI service.

- *go server* - Go server which depends on *endurox-go* package. Thus in project path the
endurox-go package must be installed. (See the sample bellow).
...
Lets assume our project will be made at /home/user1/bankapp2. The sources (with
sub-projects) will go under /home/user1/bankapp2/src. This structure is required
for Go projects.

--------------------------------------------------------------------------------
# useradd -m user1
# su - user1
$ mkdir /home/user1/bankapp2
$ mkdir /home/user1/bankapp2/src
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
$ mkdir /home/user1/bankapp2/src/ubftab
$ cd /home/user1/bankapp2/src/ubftab
...
make[1]: Entering directory `/home/user1/bankapp2/src/ubftab'
mkfldhdr -m0 -pubftab
...
U:UBF :5:  732:2ae627e394c0:000:20170211:163548263:fldhdr.c:0138:Load field dir [/home/user1/bankapp2/src/ubftab]
U:UBF :5:  732:2ae627e394c0:000:20170211:163548263:fldhdr.c:0149:About to load fields list [./test.fd,Exfields]
N:NDRX:5:  732:2ae627e394c0:000:20170211:163548264:fldhdr.c:0369:/home/user1/bankapp2/src/ubftab/./test.fd processed OK, output: ./test.fd.h
N:NDRX:5:  732:2ae627e394c0:000:20170211:163548264:fldhdr.c:0369:/home/user1/bankapp2/src/ubftab/Exfields processed OK, output: ./Exfields.h
N:NDRX:5:  732:2ae627e394c0:000:20170211:163548264:fldhdr.c:0256:Finished with : SUCCESS
make[1]: Leaving directory `/home/user1/bankapp2/src/ubftab'
make -f Mgolang
$SOURCES is [./test.fd Exfields]
...
U:UBF :5:  736:2aad91d474c0:000:20170211:163548271:fldhdr.c:0138:Load field dir [/home/user1/bankapp2/src/ubftab]
U:UBF :5:  736:2aad91d474c0:000:20170211:163548271:fldhdr.c:0149:About to load fields list [./test.fd,Exfields]
N:NDRX:5:  736:2aad91d474c0:000:20170211:163548271:fldhdr.c:0369:/home/user1/bankapp2/src/ubftab/./test.fd processed OK, output: ./test.fd.go
N:NDRX:5:  736:2aad91d474c0:000:20170211:163548271:fldhdr.c:0369:/home/user1/bankapp2/src/ubftab/Exfields processed OK, output: ./Exfields.go
...
--------------------------------------------------------------------------------
$ mkdir /home/user1/bankapp2/src/clt
$ cd /home/user1/bankapp2/src/clt
...
--------------------------------------------------------------------------------
$ mkdir /home/user1/bankapp2/src/srv
$ cd /home/user1/bankapp2/src/srv
$ go get github.com/endurox-dev/endurox-go
$ /home/user1/bankapp2/src/srv
...
Logging to ./ULOG.20170211
 ...
 1: svcname      :Service name [TESTSVC]: 
 2: useubf       :Use UBF? [y]: 
...
*** Review & edit configuration ***
 ...
 1: Edit svcname      :Service name [TESTSVC]: 
...
Compiled system type....: LINUX
 ...
 4: appHome      :Application home [/home/user1/bankapp2]: 
...
*** Review & edit configuration ***
 ...
 4: Edit appHome      :Application home [/home/user1/bankapp2]: 
...
ndrxconfig: [/home/user1/bankapp2/conf/ndrxconfig.xml]
appini: [/home/user1/bankapp2/conf/app.ini]
setfile: [/home/user1/bankapp2/conf/setapp2]


To start your system, run following commands:
$ cd /home/user1/bankapp2/conf
$ source setapp2
$ xadmin start -y
...
to Enduro/X runtime "bin" directory.

--------------------------------------------------------------------------------

$ cd /home/user1/bankapp2/bin

$ ln -s /home/user1/bankapp2/src/clt/testcl .

$ ln -s /home/user1/bankapp2/src/srv/testsv .
...
exec cconfsrv -k 0myWI5nu -i 1 -e /home/user1/bankapp2/log/cconfsrv.log -r --  :
	process id=18851 ... Started.
exec cconfsrv -k 0myWI5nu -i 2 -e /home/user1/bankapp2/log/cconfsrv.log -r --  :
	process id=18852 ... Started.
exec tpevsrv -k 0myWI5nu -i 20 -e /home/user1/bankapp2/log/tpevsrv.log -r --  :
	process id=18853 ... Started.
exec tpevsrv -k 0myWI5nu -i 21 -e /home/user1/bankapp2/log/tpevsrv.log -r --  :
	process id=18854 ... Started.
exec tmsrv -k 0myWI5nu -i 40 -e /home/user1/bankapp2/log/tmsrv-rm1.log -r -- -t1 -l/home/user1/bankapp2/tmlogs/rm1 --  :
	process id=18855 ... Started.
exec tmsrv -k 0myWI5nu -i 41 -e /home/user1/bankapp2/log/tmsrv-rm1.log -r -- -t1 -l/home/user1/bankapp2/tmlogs/rm1 --  :
	process id=18867 ... Started.
exec tmsrv -k 0myWI5nu -i 42 -e /home/user1/bankapp2/log/tmsrv-rm1.log -r -- -t1 -l/home/user1/bankapp2/tmlogs/rm1 --  :
	process id=18879 ... Started.
exec tmqueue -k 0myWI5nu -i 60 -e /home/user1/bankapp2/log/tmqueue-rm1.log -r -- -m SAMPLESPACE -s1 --  :
	process id=18891 ... Started.
exec tpbridge -k 0myWI5nu -i 150 -e /home/user1/bankapp2/log/tpbridge_2.log -r -- -f -n2 -r -i 172.0.0.1 -p 21003 -tA -z30 :
	process id=18923 ... Started.
exec testsv -k 0myWI5nu -i 2000 -e /home/user1/bankapp2/log/testsv.log --  :
	process id=18924 ... Started.
exec cpmsrv -k 0myWI5nu -i 9999 -e /home/user1/bankapp2/log/cpmsrv.log -r -- -k3 -i1 --  :
...
1  TESTSVC      TESTSVC      testsv    2000  0     0     0ms      0ms      AVAIL
1  @CPMSVC      CPMSVC       cpmsrv    9999  0     0     0ms      0ms      AVAIL
--------------------------------------------------------------------------------

TESTSVC is advertised, thus all is ok. No try will run the test client.
