...
Enduro/X build system is CMake. Version used should be 3.5 and above. It uses
Flex and Bison for building grammar parsers, such as UBF (FML) expressions.

The source code is appended with mode line settings. If using *vim*, to 
get the proper indention enabled, configure
...
Project uses 'libxml2' (provided by system), 'exhash' (already included 
...
=== XATMI server STDERR/STDOUT linking with the logger

If XATMI server binary log file is set to *stderr* in *ndrxdebug.conf(5)* (either 
file=/dev/stderr or empty file=) and *-e* flag is set in clopt for the XATMI
server process, then process level loggers (*ndrx*/*ubf*/*tp*) also logs to this
file. In such case if logrotate ("*xadmin(8)* lcf logrotate") is performed or
process level logger file name is changed via *tplogconfig(3)*), then XATMI
server's *stdout* and *stderr* is also re-directed to changed file name via
...
service requests are dispatched in round robin patter. However, if set of binaries
...
Additionally, if System V queues are not available on given platform (such as MacOS),
Enduro/X implements System V queue emulation, such mode name is *svqem*.
In *svqem* mode, all other parts for message handling are the same as described
for *SystemV* mode in this chapter.
...
=== Booting processes as XATMI servers without CLOPT
