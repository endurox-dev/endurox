...
text version of configuration and produces as close as possible matched
Enduro/X configuration files.
...
in distribution, for reference and modification purposes).

As input 'UBBCONFIG_file' or *stdin* is used. Enduro/X parses the structure of
the UBB configuration file, loads into platform script data structures, after which
