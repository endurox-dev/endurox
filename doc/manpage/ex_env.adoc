...
*NDRX_CMDWAIT*='NDRXD_COMMAND_WAIT_SECS'::
    ...
    checks. For *SystemV* mode, accuracy of this setting is 200ms (i.e. the minimum is
    given number of the seconds, but cycle in *SystemV* mode may take
    'SANITY_SECONDS'+0.2 sec between the scans).
    Default value is *1*. The sleep time between checks is not guaranteed to be this,
    as sleep might be interrupted by any incoming *ndrxd* command.
