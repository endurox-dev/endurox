...
*NDRX_XA_FLAGS*='XADMIN_XA_FLAGS'::
    ...
    *RECON*:<comma separated list of error codes e.g. 4,-8,* - any err>:<number 
    ...
    manager. Flag *FSYNC* ensures that *tmsrv(8)* log contents at commit decision are
    ...
    the distributed transaction processing, due to fact that default sync
    approach is *fflush()* Unix call, which only ensures that data for disk
    is sent to OS kernel, instead of directly to the disk. Thus default
    approach with *fflush()* poses some risk to data corruption 
