...
*NDRX_MSGMAX*='MAX_MSGS_PER_QUEUE'::
    Posix and *svqem* queue config attrib. Max number of messages that can be put in one queue.
    in case if missing, defaults to *100*.
...
*NDRX_PLUGINS*='NDRX_PLUGINS'::
    ...
    Plugins are loaded in two passes. Firstly environment variable *NDRX_PLUGINS* is
    read and plugins loaded before processing the common-configuration. If, after the
    configuration file processing, *NDRX_PLUGINS* have changed the value, addition plugin
    load attempt is done. Plugin loader loads only such shared library names, which
    where not already loaded (regardless of the first or second pass). In case if
    plugin load fails to load, process will terminate with failure.
...
*NDRX_RESPAWN*='RESPAWN_FLAG'::
    ...
    variable is not set.
*NDRX_SECURE*='SECURE_LOGGING'::
    Enable secure mode. In the current version, this affects debug logging. If set to
    *1*, sensitive data in log files are masked with asterisks. If set to *0*,
    no masking is applied. The default value is *1*.
