...
for certain activate/locked node failure and node removal from the cluster scenarios
...
lock lost *tmqueue* instance and that new activate instance would not see
...
operations on large shared file systems might be slow and may take resources
from the processing. The value such as *120* (once per two minutes) or
