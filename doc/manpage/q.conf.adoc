...
(due to failed service calls), messages are deleted from queue space. 
Prior delete message may be put in failure queue if such is set in 
*tpenqueue(3)* parameter *TPQCTL.failurequeue* field and flag *TPQFAILUREQ* is
added to *TPQCTL.flags*.
