...
[*-X*]::
If working in singleton process group, after the commit or abort decision
logging, ensure that group still holds the lock, by doing additional 
check on transaction log file existence and that file modification date is not in the future.
If this flag is not set, then only time-based check is executed, that 
singleton group lock provider has refreshed the group in time.
The *-X* can be used as an extra precaution to detect real-time clock skew
and that node thinks that it still holds the lock, 
however, the other node has already taken over. In any case if lock is lost,
*tmsrv* will exit immediately with logging the error message to log file
and ULOG. See more ex_adminman(guides)(Enduro/X Administration Manual, Active-Active considerations) section.
The downside for this flag is, that it uses a system call to check the file
the presence (stat() function). To detect clock skew, additionally *FSYNC* 
(instead of *FDATASYNC*) flag in *NDRX_XA_FLAGS* shall be used.
