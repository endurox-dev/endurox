...
tmsrv startup. Default value is *5400* (1 hour 30 min). In case if flag *-X*
is enabled, broke log file housekeeping is performed on regular bases,
instead of the *tmsrv* startup only.
...
pause/resume cycle), at small period of time there is chance of the
...
to load, it does additional attempts on next cycles (set by *-a*). This 
parameter also activates periodic housekeeping of the transaction log files
configured by setting *-h*. Housekeeping is needed to release any
prepared transactions in resource managers, for whom on disk broken transaction log
is stored, and that blocks the *tmrecoversv(8)* to zap the prepared
transactions properly.

[*-a* 'NR_OF_ATTEMPTS']::
Number of attempts used by background thread to load incomplete transaction
log files (left by any crashed process instances and not yet flushed to
file-system by the OS). Default value is *3*.
