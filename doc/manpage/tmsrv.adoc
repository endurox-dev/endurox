...
must be configured with Singleton option, so that only one node actively serves
the transactions.
...
Enduro/X does not support RAC XA Affinity and does not process 
Oracle Fast Application Notifications (FAN).
...
To overcome this limitations, careful transaction timeout planing shall be performed which
