...
'MIN_DISPTHREADS_DEFAULT'::
    This is minimums dispatch threads used by XATMI server service dispatcher.
    By default this is *1*. If set to greater value, the Enduro/X built in thread
    pool will start process incoming server messages. Note that server programs
    must be specially programmed to support multi-threading (i.e. data structures
    must be protected). If server process is booting and it detects that 
    if 'MIN_DISPTHREADS_DEFAULT' is greater than 1, then check is made against server
    process capability to handle multi-threaded work-load. By default MT dispatcher 
    is disabled in ATMI server library, and server process will fail to boot with
    corresponding message in ULOG.
    To enable MT mode, use *buildserver(8)* compiler and specify '-t' flag. Other 
    option is before starting ATMI Server main routine, set the extern variable
    '_tmbuilt_with_thread_option' to 1.
    This parameter may be overridden by 'MIN_DISPTHREADS_SERVER'.
'MIN_DISPTHREADS_DEFAULT'::
    This parameter is reserved for future use. If 'MIN_DISPTHREADS_DEFAULT' (or
    'MIN_DISPTHREADS_SERVER') is being configured, then this parameter must be 
    greater or equal to 'MIN_DISPTHREADS_DEFAULT' (or 'MIN_DISPTHREADS_SERVER')
    value. If this is not a true, then server process will fail to boot with
    corresponding message in ULOG.
'AUTOKILL_DEFAULT'::
