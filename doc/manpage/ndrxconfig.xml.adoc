...
*PROTECTED_SERVER_DEFAULT*::
	Protected server is one that does not shuttdown with 'xadmin stop' unless you pass the
...
*ATMI_SERVER_EXPORT_SERVICES*::
	...
	*tpbridge* servers.
*ATMI_SERVER_BLACKLIST_SERVICES*::
	EnduroX server specific list of services that must not be exported. This list is only for 
