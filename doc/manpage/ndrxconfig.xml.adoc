...
'ATMI_SERVER_SYSTEM_OPTIONS'::
    ...
    *-S*'<service_name1>/<service_name2>[:<function name>]' flag. Functions are
    ...
    that stdout & stderr of server is dumped to 'LOG_FILE'. *-o* specifies *stdout*
    output file. Note that if *stdout* is configured, it is not managed by Enduro/X
    logger. There are few internal parameters: parameter *-k* is just a random key
