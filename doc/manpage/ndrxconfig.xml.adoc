...
'ATMI_SERVER_SYSTEM_OPTIONS'::
    ...
    all service advertise and with -s some single service may be enabled. With
    flag *-z* <RTGRP> it is possible to define or override the env variable 
    *NDRX_RTGRP* - routing group code. Use *-Z* flag to configure that particular server
