...
'NDRX_CMDWAIT'::
    ...
    checks (such as sanity scans). For *SystemV* mode, accuracy of this
...
'SANITY_SECONDS'::
    ...
    setting depends on the *NDRX_CMDWAIT* value. As this actually
    is time by which *ndrxd(8)* sleeps periodically between the checks.
'CHECKPM_STY'::
