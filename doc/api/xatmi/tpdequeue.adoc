...
for message, how it shall be dequeue and additional metadata. Also 'ctl' 
returns diagnostics information in case of error. 'ctl' must be fully initialized before use e.g.

---------------------------------------------------------------------------------

TPQCTL qc;
memset(&qc, 0, sizeof(TPQCTL));

/* qc parameter setup and function call... */

---------------------------------------------------------------------------------

'data'/'len' pair is XATMI  buffer to which dequeued
message must be written. Buffer will be reallocated 
...
It is not possible to enqueue and dequeue the same message within same global transaction.
...
If message is dequeued, the same 'ctl' is returned as used for *tpenqueue(3)*, except:

- Field 'qdeq_time' value is not returned (keeps value passed to tpdequeue() call).

- Field 'qexp_time' value is not returned (keeps value passed to tpdequeue() call).    

- Field 'flags', if flag *TPQTIME_REL* was specified, it is changed to *TPQTIME_ABS*.

- Field 'diagnostic' is set to *0*.

Additionally, the field value of 'ctl->cltid' is retained from the original
enqueue with the message stored in the reply queue (field 'replyqueue', flag *TPQREPLYQ*)
and failure queue (field 'failurequeue', flag *TPQFAILUREQ*) or the
failure queue specified by the *errorq* setting in *q.conf(5)*.
