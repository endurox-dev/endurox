...
DESCRIPTION
-----------
This function copies fields from 'p_ub_src' to 'p_ub_dst' buffer. The list of field which needs to be copied is marked in 'fldlist' array. The array must be terminated with *BBADFLDID*. Matchied field names are copied including all it's occurrences. Before performing copy, 'p_ub_dest' buffer is erased by Binit(3).
