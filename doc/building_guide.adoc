...
# yum install git cmake asciidoc openssl openssl-devel \
	gpgme-devel redhat-lsb libxml2-devel
---------------------------------------------------------------------
...
In case if additional repositories are configured in the system such es 
EPEL (e.g. CentOS 8 Package Management), then gpgme-devel can be installed too:
...
# yum update
# yum install gpgme-devel gpgme
---------------------------------------------------------------------

Install GPG-ME on Oracle Linux 8.x:

---------------------------------------------------------------------
$ su - root
# dnf config-manager ol8_codeready_builder
# dnf --enablerepo=ol8_codeready_builder install gpgme-devel gpgme

---------------------------------------------------------------------

If GPG-ME is not configured, then build with *-DDEFINE_DISABLEGPGME=ON*.
