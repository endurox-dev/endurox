...
located in /opt/app1 folder. With follwing direcotry structure:
...
---------------------------------------------------------------------
This is ok, we have configured two copies of eserver Enduro/X servers, 
...
Then this typically means, that you do not have run /etc/rc.local (either by 
root or by reboot). More info is logged to /opt/app1/log/xadmin.log
