...
process calls some service X, and it gots valid results back, then data key is
...
Cache supports different features, for example:

. Drop the cache at Enduro/X boot time.

=== Limitations of the chache

The LMDB is build in such way way that if write transaction on the database is
open, then other writes will not be able to process it in meantime. While read
only transactoins are processed, while some other process holds write transactoin.
...
have to sync in that place - thus makes point of bottlenc.
