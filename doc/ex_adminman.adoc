...
*NDRX_MSGMAX* setting from the *ex_env(5)*. The default is limit *64*. Hard limit
is *99999*.

Singleton process groups does not guarantee 100% probability that processes
will run only at the same time on one Enduro/X server instance. There could
be cases, such as VM suspend and resume, when current node was removed from
...
other shared file system clusters, please consult with corresponding vendor.
...
If processes such as *ndrxd* and *cpmsrv* detects at sanity cycles that
time-stamp refresh has not happened in given time (*NDRX_SGREFRESH* from
*ex_env(5)*), the group in shared memory is unlocked, and *ndrxd*/*cpmsrv*
proceeds with killing all the group processes. For time stamping *CLOCK_REALTIME*
the clock is used, which reports to the system real-time, thus inactivity or
system freezes can be detected by this clock. However, false-positives are possible,
if for example administrator changes the clock setting on the servers.
In such cases, if changes cover larger gaps of time than specified
in *NDRX_SGREFRESH* parameter, the group will unlock and failover will
happen. It is recommended to keep the clocks accurate and let the *NTP*
daemon on a regular bases, (e.g. few times a hour - depending on the hardware
and software environment), to sychrnoize the clocks.

- *exsinglesv* regularly verifies the lock status on shard FS and additionally
employs the second "ping" lock file, which on a regular basis is locked and
...
By design, transaction logs of the *tmsrv(8)* are not replicated to other nodes.
Thus if active cluster node fails, the other nodes cannot complete transactions
started on the crashed node. However, transaction manager logs may be stored
on shared file system and *tmsrv(8)* instances may be configured in the
sinegton groups. In such case failover can happen to other nodes, for which
if properly configured, the transaction manager will be booted back, it will
recongnize the transaction logs from crashed node, and will perform
distributed transaction completing.

Following configuration settings must be set for the transaction manager:

- Shared "virtual" Enduro/X cluster node ID shall be set for *tmsrv* instances
which may took the failover. Command line parameter *-n*. Which shall be
set to any node number from the cluster.

- Transaction manager instances in different Enduro/X nodes must match
...
Transaction manager server *tmsrv(8)* for the particular resource manager shall
run only in one copie 
...
==== Sample configuration fof Active-Active setup with singleton groups
