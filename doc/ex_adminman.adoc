...
//@param node is server node (i.e. $node) on which script is exectued
...
sub NdrxState2Num(node, parameter)
{
...
can be created. For demo purposes following DCI items will be created:

- Total processed items to see the system TPS;

- Total successful transactions for some service;

- Total failed transaction for some service;

- Total number of servers for some service;

To create *DCI*, find the host in *Objects* navigator, in Context menu chose
*Data Collection Configuration* and press *New parameter* (either in Context menu
or on right top side):
