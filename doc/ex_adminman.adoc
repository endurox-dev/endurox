...
Defaults values for *exsinglesv* (with default *NDRX_SGREFRESH* env value) are:

- Check interval *10* seconds (*chkinterval*). However may be reduced, for extra
...
- Another node Enduro/X signleton process group boot, after the failover (shared file system has
...
re-configuring time-outs/settings, plus this wait time (e.g. *6* cycles multiplied by *10* sec).
...
reason, that would give tie *ndrxd*/*cpmsrv* processes to kill the group.
