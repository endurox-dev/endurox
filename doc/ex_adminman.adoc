...
to configure that set of processes in a cluster of several machines run only on one
machine at a time, meanwhile if the active machine goes out of order, then failover happens
...
In the event of node failover, the next node will acquire the lock and after a certain
...
Singleton process groups do not guarantee 100% probability that processes
will run only at the same time on one Enduro/X node instance. There could
be cases, such as VM suspend and resume, when the current node was removed from
the shard FS cluster, the other node did got the shared file system lock and the group
was booted on the new node. Meanwhile, for some reason VM on the old node was resumed,
the old node *exsinglesv*/*ndrxd*/*cpmsrv* will detect that group lock was
lost and group processes will be killed, but this detection requires at least
about *2* sanity cycles of *ndrxd*/*cpmsrv*, meaning that during that time
singleton processes will run on two nodes at the same time. To ensure that
this never happens, the *STONITH* device must be configured. For example *GFS2*
...
time-stamp refresh has not happened in the given time (*NDRX_SGREFRESH* from
*ex_env(5)*), the group in shared memory is unlocked, and *ndrxd*/*cpmsrv*
proceeds with killing all the group processes. For non-Linux based systems,
for time-stamping *CLOCK_REALTIME* clock is used,
which reports to the system real-time, inactivity or
system freezes can be detected by this clock.

However, the *CLOCK_REALTIME* clock value can be manually
changed by the system administrator, or it can be changed by NTP.
In such cases, if clock changes are done with large gaps of time,
larger than the time specified in *NDRX_SGREFRESH* parameter,
then groups will be unlocked and failover will
happen. It is recommended to keep the clocks accurate and let the NTP
daemon synchronize clock on a regular basis with small interval bases
(e.g. several times per hour - depending on the hardware and software environment).

For Linux systems, *CLOCK_BOOTTIME* is used, which is not affected by
adjustment problem.

- *exsinglesv* regularly verifies the lock status on shard FS and additionally
employs the second "ping" lock file, which regularly is locked and
unlocked. If fore some reason there is an issue with the lock check or "ping" lock
sequences, the singleton process group is unlocked and *ndrxd*/*cpmsrv* will
proceed with kill group binaries.

Additionally, when operating with process groups, it is not recommended to
...
==== Distributed transactions in A-A environment

The full life-cycle of the transaction is managed by particular
transaction manager *tmsrv(8)* instance which did start the particular
...
environment, the failover functionality may be confiugred for the *tmsrv*. So that,
if node had active transaction managers, but for some reason node
...
for strong the transaction logs. The first step of configuring failover, transaction
manager shall store it's logs on (i.e. CLI paramter *-l* shall point to) shared file system, such
as *GPFS* or *GFS2*.

Second step for implemeting failover for transaction managers, is to configure
the singleton groups for *tmsrv(8)* instances. Additionally instances on different
Enduro/X nodes, shall point to the same transaction log directory. As transaction
logs include information about the Enduro/X cluster node id and <srvrid> values on
which transaction was started, it is crutial that *tmsrv(8)* on different Enduro/X
nodes matches following attributes:

- Common virtual Enduro/X cluster node id shall be set for matching *tmsrv(8)* instances.
Value shall be passed to the *tmsrv* parameter *-n*. Which shall be 
...
At the end of each singleton group it is recommended to configure the *tmrecoversv(8)*.
This will ensure that any orphaned prepared 
transactions branches are collected (rolled back). For failover groups,
it is recommended to use *-i* (immediate) flag, so that *tmrecoversv* is done
...
guarantee transaciton log file storage is not used.

*tmsrv(8)* shall be configured to use *FSYNC* (or *FDATASYNC*) and *DSYNC* flags
in the *NDRX_XA_FLAGS* env, so that commit decision is fully logged to storage
device. In case if planning to use *tmsrv(8)* *-X* flag for clock scew detection
against the log files, then *FSYNC* and *DSYNC* shall be used, instead of the *FDATASYNC*
and *O_DSYNC*, as *FDATASYNC* does not flush log file modification dates to the
file system. However, *FSYNC* and *-X* paramter may have performance impact on the
...
==== Sample configuration of the Active-Active setup with singleton groups
