...
Singleton process groups does not guaratee 100% probability that processes
...
shard FS cluster, other node did got the shared file system lock and group
was booted on new node. Meanwhile, for some reason VM on old node was resumed,
...
this never happens, *STONITH* device must be configured. For example *GFS2*
requires *STONITH* as part of the shared file system installation. For
other shared file system cluster, please consult with correspoding vendor.

Singleton groups keeps the following health-checks:

- Each group in shared memory keeps the refresh time-stamp interval.
the *exsinglesv* task is periodically refresh the time-stamp in shared memory.
If processes such as *ndrxd* and *cpmsrv* detects at sanity cycles that
time-stamp refresh has not happend in given time (*NDRX_SGREFRESH* from
*ex_env(5)*), the group in shared memory is unlocked, and *ndrxd*/*cpmsrv*
proceeds with kill all the group processes. For time stamping *CLOCK_REALTIME*
clock is used, which reports to the system real-time, thus inactivity or
system freezes can be detected by this clock. However false-positives may,
if for example administrator changes the clocks setting on the servers.
In such case, if changes cover larger gaps of time than specified
in *NDRX_SGREFRESH* parameter, group will unlock and failover will
happen. It is recommend to keep the clocks accurrate and let the *NTP*
daemon on regular bases, (e.g. few times in a hour - depending on hardware
and softare environment), to sychrnoize the clocks.

- *exsinglesv* regulary verifies the lock status on shard FS and additionally
employs the second "ping" lock file, which on regular basis is locked and
unlocked. If fore some reason there is issue with lock check or "ping" lock
