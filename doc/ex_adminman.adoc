...
However, message store must be placed on the shared file system.
...
which means that on a node which took over, the transaction data might not be
available.
...
to the shared file system (including file rename, unlink), open new handles, write
...
In an ideal scenario, *STONITH* shall be employed, so that if node fails,
...
always reliable, as, if VM is paused, then resumed the time during
the pause/resume cycle by some VM hypervisors, time does not move forward during the pause,
resulting in that time lease is not expired on the failed node, but assumed to be expired by other nodes.
...
and *mmfsck* on partition might be required to mount the partition back. Additionally,
...
other such similar kind of errors) actually can happen in the given environment.
...
*GPFS* for SCSI-3 Persistent Reservation fencing. Additional benefit from
...
Following key Posix file system properties must be met by shared file system,
...
- When new file is created, and program did Unix call fsync() on file descriptor there must be
guarantees, that file appears in directory after the fsync() return successful results.
...
- When program issues rename() Unix call on the file, operation is atomic and new path is
...
- When program issues unlink() Unix call to the file, the operation must be atomic and
persisted to the disk after unlink() successfully returns. For Linux 
"DSYNC" Enduro/X flag ensures that Linux synchronises the directory
to the final state.

If above features are not provided by the Operating System or Shared File System,
at point of crash and failover, there might be a chance for transaction
loss.
...
Firstly let's create the script which will be called on the event. Script
...
-- 1) In following command replace "user1" with application user.
...
To enable Persistent Reservation for the GPFS, use following command:
...
*pr=yes*, means that persistent reservation feature is enabled.
...
nodes cluster, for simplicity of administration and better fault tolerance,
three node clusters are recommended. All node shall have access to the shared disk storage.

As for the Enduro/X application it self, it may run on two nodes, and third node
may be used as a quorum node only.

Typical topology of the Enduro/X cluster would look like following:
...
*FibreChannel* (FC) or *ISCSI*. FibreChannel method is preferred way of connecting
disks. For Fibre Channel, link speeds typically would be *16 Gbps* or *31 Gbps*.
For ISCSI link speed typicall is *10G* or *25G*.

For the performance requirements, disk models, disk array, configuration guides
and what is supported with chosen shared file system,
...
fence support, consult with the IBM on which devices on GPFS support given feature.

When choosing the hardware for transaction manager (*tmsrv*) and transactional
message queue support (*tmqueue*), the key performance aspect for the hardware,
is the number of fsync() operations per second disk infrastructure is capable of.
fsync() is command which issues write to disk and returns only when data is actually
...
it has been shown that Enterprise level battery backed-up RAID controllers has
better fsync() performance, than drives without battery backup. Enterprise
level SSD drivers might also show better results, but that depend on the
vendors, and the results might be significantly different between different
HW vendors.

Typically Enduro/X has following fsync numbers:
...
However fsync() is not the only parameters which affects the application
performance.
...
Directory for *tmqueue(8)* is configured in the XA open string for *libndrxxaqdisks(8)*.
...
commit decision logging and right after decision successfully logged. For
TMQ transactions, additional checks are made when when TMQ transaction is
started and at point where it is being prepared.
...
configured, which is located on the both machines. By default *64* process may
be configured, and they may be configured in the same way as described here.

Process group "RM1TMQ" will be configured in the example, allocate on nodes
*1* and *2*.
...
Any node id (NDRX_NODEID) used in cluster. Shall be common for all participants in the cluster
|-X<seconds> | Number of seconds tor run for disk checks
(i.e. scan for unseen transaction logs, left from other node if had concurrent run). However
...
Corrupted logs might happen, if active node just opened transaction log and node crashed.|*90*
|=========================================================
...
|-n<node_id> | Common cluster node id, used for log file recognition|
Any node id (NDRX_NODEID) used in cluster. Shall be common for all participants in the cluster
|-X<seconds> | Number of seconds to run for disk checks (i.e. scan for non
loaded messages, left from other node if had concurrent run). Normally shall not happen,
if proper fencing is used|*60*
|=========================================================

Additional notes:

If expecting large number of messages in persistent queue (>5K), please increase
...
would allow boot sequence to wait for message loading into memory queue space,
before moving on with next servers to boot.
...
|=========================================================
|Parameter|Description|Recommended value
|-z<zero_snd_time> | Interval into which zero length message is sent to socket to keep the
activity|*5*
|-c<check_time> | Number of seconds for check intervals. This interval checks
for running -z message sending|*2*
|-a<rcv_time> | Number of second into which something must be received from
the socket, if not received, connection is closed|*10*
|=========================================================

Above configuration will ensure, that socket will be reset after 10 seconds,
...
- Maximum refresh time, by which local node will unlock is *30* seconds (NDRX_SGREFRESH).
I.e. *exsinglesv* must have run at-least one check cycle in this time.

- Other node Enduro/X signleton process group boot, after the failover (shared file system has
released *lockfile_1* to other node), is *60* seconds (locked_wait). Meaning that
total time for survived node to boot would depend on shared file system
re-configuring time-outs/settings, plus additionally this wait time. The wait
...
When node 2 is started, following output is expected from the system (sample output):
...
After the startup XATMI server process states can be inspected by following
command:
...
With following command lock groups can be inspected for the current application
running:
...
- *PROCGRPL* - This process is lock provider for given group (group name).

- *PGLNO* - This process is lock provider for given group (group number).

- *PGNLA* - This is reported group number for which lock provider is serving.
...
To avoid initiation of the failover (good node takes bad node's lock and boots the group),
...
After enabling such command, node will not take new locks / perform fail-over.
...
To check singleton group statuses on the given machine, following command may be used:
