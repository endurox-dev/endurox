...
manager shall store it's logs on shared file system, for example *GFS2*
or any other FS which has Posix semantics. Logs directory for *tmsrv* is set by
*-l* parameter. The Posix semantics here are understood as:
...
for example *GFS2*. The share file system shall support Posix semantics, such as:

- fsync() or fdataync() unix calls after the return guarantees that data is written to the file,
even after the system crash.

- When new file was created, and program did Unix call fsync() on file descriptor there must be
guarantees, that file appears in directory after the fsync() return results (even after
the system crash).
...
For other OS and/or file systems, please consult with the third party vendors, do they support
such mode.

If above features are not provided by the Operating System or Shared File System,
at point of crash and failover, this might lead to loss of transactions (i.e.
partial commit and partial rollback of the transaction branches).
...
- Active node is so busy of work and becomes unresponsive (basically server overloaded),
so that shared file system cluster may decide that node failed (unresponsive), and it does
...
way to fence, as that would also ensure that user application processes are
...
which shall be used by third shared file system, if they support such feature.
In this case, at hardware level shared disk is disconnected from the failed node.
*Requirement is, that no write happens from the failed node after the node failed.*
Before shared disk becomes available on failed node for write, the user application must
be killed, to avoid any accidental writes. The killing can be done by
issuing "$ sudo xadmin killall <application_user>" command. *Immediately node shall
be rebooted, right before shared disk becomes mounted.*

If not using HW based fencing, that might *cause corruption* of the
...
*loose transactions*, *loose persistent queue messages* and
other data if storing them in shared fs storage.
...
seconds, thus failed node cannot detect that its time lease have expire, it
it might overwrite disk structures in non-synchronised way, leading to corruption.
...
- When program issues rename() Unix call on the file, operation is automatic and is
final right after the rename() returns the result. I.e. file has changed location,
and would be there even after the system crash. For Linux "DSYNC" Enduro/X flag
ensures that Linux synchronises the directory to the final state. For other OS
and/or file-systems, please consult with the third party vendors, do they support
such mode.

- When program issues unlink() Unix call to the file, the operation must be atomic and
final right after the unlink() returns the result (i.e. file is removed even after
the system crash). For Linux "DSYNC" Enduro/X flag ensures that Linux synchronises
the directory to the final state. For other OS and/or file-systems,
please consult with the third party vendors, do they support such mode.

If above features are not provided by the Operating System or Shared File System,
at point of crash and failover, this might lead to loss or double of the
persistent messages. I.e. persistent message in the queue is lost or
dequeued message from the queue appears back in the queue.
...
file system shall provide such functionality (e.g. *GFS2*) and it must
...
Corrupted logs might happen, if active node just opened transaction log and node crashed.|120
|=========================================================
...
<endurox>
    ...
    ...
        ...
        <server name="tmsrv">
            ...
            <srvid>60</srvid>
            <cctag>RM1TMQ</cctag>
            <sysopt>-e ${NDRX_ULOG}/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X2 -r30 -h120</sysopt>
        ...
        <server name="tmqueue">
            ...
            <srvid>80</srvid>
            <cctag>RM1TMQ</cctag>
            <sysopt>-e ${NDRX_ULOG}/tmqueue-rm1.log -r -- -s1 -p30 -u30 -n2 -X60 -f100</sysopt>
            <procgrp>RM1TMQ</procgrp>
...
exec tmsrv -k 0myWI5nu -i 60 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X2 -n2 -r30 -h180 --  :
	process id=3512 ... Started.
exec tmsrv -k 0myWI5nu -i 61 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X2 -n2 -r30 -h180 --  :
	process id=3524 ... Started.
exec tmsrv -k 0myWI5nu -i 62 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X2 -n2 -r30 -h180 --  :
...
exec tmsrv -k 0myWI5nu -i 60 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X2 -r30 -h180 --  :
	process id=0 ... Waiting on group lock.
exec tmsrv -k 0myWI5nu -i 61 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X2 -r30 -h180 --  :
	process id=0 ... Waiting on group lock.
exec tmsrv -k 0myWI5nu -i 62 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X2 -r30 -h180 --  :
	process id=0 ... Waiting on group lock.
exec tmqueue -k 0myWI5nu -i 80 -e /home/user1/test/log/tmqueue-rm1.log -r -- -s1 -p30 -u30 -n2 -X60 -f100 --  :
	process id=0 ... Waiting on group lock.
exec testsv.py -k 0myWI5nu -i 1250 -e /home/user1/test/log/testsv.py.log -r --  :
	process id=0 ... Waiting on group lock.
exec testsv.py -k 0myWI5nu -i 1251 -e /home/user1/test/log/testsv.py.log -r --  :
