...
in cooperation with Mavimax have implemented *POLLEXCL* flag in AIX. With this 
...
variable is set to *LIFO:ONE* (or minimum to *ONE*. The *ndrxd(8)* sets
*POLLEXCL_POLICY* automatically at XATMI server startup (after the fork before exec) 
if value was not previously set. Global shell settings are not needed 
(unless for some special reason, e.g. server started outside *ndrxd*, but note, 
that setting *LIFO:ONE* at shell level, causes Enduro/X fail to boot, but *ONE* boots ok. 
This seems to be some AIX kernel specific issue). If for some reason other value needs to
be used for the XATMI server process, that can be set at *<envs>/<env>* tag 
in *ndrxconfig.xml(5)*. The *POLLEXCL* usage can be disabled for particular 
process by setting at *NDRX_NOPOLLEXCL* to *Y* at *<envs>/<env>* tag 
(Thought this is not recommend, and it will introduce thunder herd issue, 
if several same type binaries are started).
...
- Bytes 6-9: Sequence counter, start value is randomized during process init, in network order.

- Bytes 9-14: Unix echo time stamp in seconds, in network order.
...
The CID guarantees that 4 billion transaction IDs/TMQ IDs per second would
