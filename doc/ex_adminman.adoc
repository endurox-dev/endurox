...
When transaction is about to commit, *tmsrv(8)* write the decsion to persist storage.
For transaction consistency it is crutial, that if active node did fail,
and shared file system gave fcntl() singleton-group lock to other node,
*no write must happen from failed node to shared storage after the failover*.
If such writes happen, transaction logs might become corrupted and
Enduro/X transaction manager *may loose transactions*.

Example cases, wgeb write from failed node might happen:

- Virtual machine with currently active node is suspended, shared file
system assumes that node failed and leaved the cluster, the shared file
system manager gives fcntl() lock (used by singleton groups) to failover
node which takes over, and then suspended node is resumed.

- Currently active node is so busy of work and becomes unresoponsive,
that shared file system cluster may decide that node failed, and it does
the failover. When the failed node load is back to normal, it starts
to write to disk again.

To resolve such scenarious, hardware level fencing is required.
In ideal scenario, *STONITH* shall be eployed, so that if node failed,
...
whcih shall be used by shared file system. In this case, at hardware level
...
shared file system by it self. In such case Enduro/X cluster might stop
to work and loose transactions, including the persistent messages,
if storing them in shared fs storage.
