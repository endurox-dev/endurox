...
As IBM AIX also does not have equivalent functionality of the Linux *EPOLLEXCLUSIVE* flag, 
IBM AIX is prone to the same CPU wasting at specific workloads as with FreeBSD
(described above). Mavimax have made change-request to IBM for adding Linux
like poll-exclusive flags (https://www.ibm.com/developerworks/rfe/execute?use_case=viewRfe&CR_ID=144545).
...
<dict>
 ...
 <string>kern.sysv.shmall=102400</string>
...
Random is generated by rand_r(), start is randomized by time/pid/uid/time.
The CID guarantees that 4 million transaction IDs/TMQ IDs per second would
