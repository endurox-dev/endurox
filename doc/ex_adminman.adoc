...
- For distributed transaction *tmsrv(8)* after commit decsion logged,
verifies that it is still the locked node. By default it only verifies
local node singleton group lock status. However, if *STONITH* device
is not used for the cluster (which guarantees that fcntl() on new
node does not get the lock unless *STONITH* of broken node has happened),
settings *sg_nodes_verify* must set be to *Y*, so that lock status
is checked cluster wide. As parallel runs of *tmsrv* may affect
the global transaction consistency.

- If *STONITH* device is not used in the cluster, *tmsrv* and *tmqueue*
processes have new CLI setting *-X<seconds>*. This setting is used
to detect parallel run of the *tmsrv* and *tmqueue* processes, to
rocover from cases when for newly activated node, due to parallel
run, new transactions or enqueued messages are not detected. If
flag is enabled, processes periodically scan the data folders,
and if such condition is found, processes reboot to perform
full data read from the disks. By default setting is not applied,
but if *STONITH* is not used, setting shall be enabled. Read
more about them (full meaning and recommended values)
in corresponding process man pages.
...
It is recommended to put another *tmrecoversv* instance at the end of the application
configuration on each node, which runs periodically. That will allow to housekeep
any invalid prepared transactions, which in case of missing *STONITH* device, might
cause lock-loosed node for small period of time to generated such transactions
for whom activate node is unaware.
