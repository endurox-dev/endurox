...
any orphaned prepared transactions for whom corrupted transactions logs
...
Additional details for working with specific resource managers are outside of scope
of this guide.
...
-- 2) Replace "/home/user1/soft/conf/settest1" with environment file of the application.
...
su - user1 -c "source /home/user1/soft/conf/settest1; xadmin down -y"
exit 0
...
specs, has about 500K random write IOPS and Intel S4600 SSDs has 72K random write IOPS,
...
on empirical bases, by doing benchmarks on different configurations.
...
However, if *STONITH* is not available on given shared file system, Enduro/X 
transaction manager still can keep the distributed transaction consistency,
with requirement, that at the event of shared fs cluster failure, fs becomes unavailable
or read only.

Singleton groups keeps the following health-checks:

- Each group in shared memory keeps the refresh time-stamp interval.
the *exsinglesv* periodically refreshes the time-stamp in shared memory (monotonic clock
based time-stamp).
...
messages appears in Q-space disk so that queue-space on active
node can be consistently read messages from the disk. *-X* setting for
*tmsrv* also lets to remove expired/broken transactions logs,
...
|-r<attempts | Number of retries to try to finish the transaction if resource managers are not working.
A higher value is recommended, if *tmsrv* serves for *tmqueue*, which might contain
a large amount of messages. The actual completion time is multiplied by *tmsrv*
background scan time *-s*|*30*
...
committing phase. On the new node, some records in the resource (e.g. DB, TMQ) might stuck in prepare
phase, until housekeep happens.|*60*
|=========================================================
...
would ensure that the if lock provider lost the lock on the other node for some
...
Follwing ini-based common configuration settings for all cluster nodes which
...
<endurox>
    ...
    ...
        ...
        <server name="tmsrv">
            ...
            <sysopt>-e ${NDRX_ULOG}/tmsrv-rm1.log -r -- -t60 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X10 -r30 -h60</sysopt>
        ...
        <server name="tmqueue">
            ...
            <srvid>80</srvid>
            <cctag>RM1TMQ</cctag>
            <sysopt>-e ${NDRX_ULOG}/tmqueue-rm1.log -r -- -s1 -p30 -u30 -n2 -X60</sysopt>
            <procgrp>RM1TMQ</procgrp>
...
exec tmsrv -k 0myWI5nu -i 60 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X10 -n2 -r30 -h60 --  :
	process id=3512 ... Started.
exec tmsrv -k 0myWI5nu -i 61 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X10 -n2 -r30 -h60 --  :
	process id=3524 ... Started.
exec tmsrv -k 0myWI5nu -i 62 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -X10 -n2 -r30 -h60 --  :
	process id=3536 ... Started.
exec tmqueue -k 0myWI5nu -i 80 -e /home/user1/test/log/tmqueue-rm1.log -r -- -s1 -p30 -u30 -X60 -n2 --  :
...
exec tmsrv -k 0myWI5nu -i 60 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X10 -r30 -h60 --  :
	process id=0 ... Waiting on group lock.
exec tmsrv -k 0myWI5nu -i 61 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X10 -r30 -h60 --  :
	process id=0 ... Waiting on group lock.
exec tmsrv -k 0myWI5nu -i 62 -e /home/user1/test/log/tmsrv-rm1.log -r -- -t1 -l/kvmpool/data/test/tmlogs/rm1 -n2 -X10 -r30 -h60 --  :
	process id=0 ... Waiting on group lock.
exec tmqueue -k 0myWI5nu -i 80 -e /home/user1/test/log/tmqueue-rm1.log -r -- -s1 -p30 -u30 -n2 -X60 --  :
	process id=0 ... Waiting on group lock.
exec testsv.py -k 0myWI5nu -i 1250 -e /home/user1/test/log/testsv.py.log -r --  :
	process id=0 ... Waiting on group lock.
exec testsv.py -k 0myWI5nu -i 1251 -e /home/user1/test/log/testsv.py.log -r --  :
