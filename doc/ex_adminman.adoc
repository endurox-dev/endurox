...
Following key Posix file system properties must be met by the shared file system,
...
guarantees, that file appears in directory after the fsync() returns successful results.
For Linux "DSYNC" Enduro/X flag can be set which opens the
directory in read-only mode and performs fsync() on directory file descriptor.
...
the "$ps -ef" pattern in grep style, extracts PIDs, and kills them with -2, -15, -9
signals, this is done from the *root* user. After that from the application user and
application environment, the *xadmin down* is called to ensure that all 
shared resources from the application are released.
...
node cluster, for simplicity of administration and better fault tolerance,
three node clusters are recommended. All nodes shall have access to the shared disk storage.

As for the Enduro/X application it self, it may run on two nodes, and the third node
may be used as a quorum node only.
...
As for the disk connectivity, the shared disk array may be connected via
...
fsync() is a command which issues write to disk and returns only when data is actually
...
always correlated with the fsync() numbers). In classic RAID setups,
it has been shown that Enterprise level battery-backed RAID controllers have
better fsync() performance, than drives without battery backup. Enterprise
level SSD drivers might also show better results, but that depends on the
vendors and the results might be significantly different between different
HW vendors.
...
transaction manager still is able to keep the distributed transaction consistency,
with requirement, that at event of shared fs cluster failure, fs becomes unavailable
or read only.
...
has higher counter value, then the given node performs unlock and after that
*ndrxd* and *cpmsrv* steps in to kill the group processes.

- In case if *STONITH* is not used *tmsrv(8)* after each commit decision logged,
...
commit decision logging and right after decision is successfully logged. For
...
node can be consistently read from the disk.
...
The following diagram depict the core functionality of the singleton group
...
|-X<seconds> | Number of seconds tor run for disk checks
(i.e. scan for unseen transaction logs, left from another node if had concurrent run). However,
if proper fencing is used (STONITH or SCSI-3 Persistent Reserve), this parameter shall have no effect.|*10*
|-r<attempts | Number of retries to try to finish the transaction if resource managers are not working.
Higher value is recommended, if *tmsrv* serves for *tmqueue*, which might contains
large amount of messages of message. The actual completion time is multiplied by *tmsrv*
background scan time *-s*|*30*
|-h<seconds> | Number of seconds to wait for housekeeping to remove corrupted transaction logs.
Corrupted logs might happen, if the active node just opened the transaction log and the node crashed.|*60*
|=========================================================
...
left from other node if had concurrent run). Normally shall not happen,
if proper fencing is used|*60*
|=========================================================
...
would allow the boot sequence to wait for the message loading into the memory queue space,
before moving on to next servers to boot.
...
for *GPFS* setups, as read file access looks to be faster than network
access with this shared file system. Default is *0* - disabled (use @SGREM
for remote node status/counter reading
|=========================================================

Defaults (and recommended) values for *exsinglesv* (with default *NDRX_SGREFRESH* env value) are:
...
I.e. *exsinglesv* must have run at-least one check cycle in this time.
...
total time for the survived node to boot would depend on shared file system
re-configuring time-outs/settings to release fcntl() locks,
plus this wait time (e.g. *6* cycles multiplied by *10* sec).
The wait time shall be at-least two times higher than other node refresh time, as that
would ensure that if-lock provider lost the lock on the other node for some
reason, that would give time to *ndrxd*/*cpmsrv* processes to kill the group.
And only after that time, new node would boot the processes back on.

==== INI configuration part

For demonstration purposes common configuration file for common-configuring
environment will be defined here. Additional configuration file for each
    ...
    ...
        ...
        <server name="tmqueue">
            ...
            <!-- Increase time for start wait, as there might be lots of messages
                 to load*
             -->
            <start_max>100</start_max>
            <!-- do not start next server within startup time -->
            <srvstartwait>100</srvstartwait>
        </server>
...
*mmoff* command puts node back to normal.
...
*7* - Corrupted structures. *8*  - Locked by network rsp (other node has got the lock),
