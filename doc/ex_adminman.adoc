...
- When a new file is created, and the program did Unix call fsync() on file descriptor there must be
...
signals with additoinal *ndrxd* killing first, to avoid respawnwing, 
...
shared resources from the application are remove. Note that if disk failure
happens, the application on broken node is terminated, and shall be booted
back manually. It might be possible to use *mount* callback, to start
application back, however that's outside of the scope of this document, and
at such crash as FS loss, server reboot would be recommended, where at startup
...
messages appear Q-space disk, so that queue-space on active
node can be consistently read from the disk. *-X* setting for
*tmsrv* also lets to remove expired / broken transactions logs,
which for some scenarious allows *tmrecoversv(8)* to remove
...
(i.e. scan for unseen transaction logs or housekeep borken records after their expiry).|*10*
|-r<attempts | Number of retries to try to finish the transaction if resource managers are not working.
A higher value is recommended, if *tmsrv* serves for *tmqueue*, which might contains
...
Corrupted logs might happen if the active node opened the transaction and did not passs till the
committing phase. On new node, some records in resource (e.g. DB, TMQ) might stuck in prepare
...
however it is recommended ot leave this setting for any failover setup.|*60*
|=========================================================
...
reason, two times higher value would give time the other node's 
*ndrxd*/*cpmsrv* processes to kill the group. And only after that time, 
new node would boot the processes back on.

==== INI configuration part

For demonstration purposes common configuration file for common-configuring
environment will be defined here. Settings in in-file lists only new
entries needed for the singleton group configuration.
