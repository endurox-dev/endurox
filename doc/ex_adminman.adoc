...
way is to use *ONE* or (*ONE:RR* as *RR* is by default). The *LIFO* or *FIFO*
settings seems currently on AIX causes only first/last polled process on Qs
picks ups the messages. The *POLLEXCL* usage can be disabled for particular 
