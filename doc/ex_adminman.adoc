...
happen. To keep the server time accurate, it is recommended to perform NTP
clock synchronization on a regular basis with small check/sync intervals
(e.g. several times per hour - depending on the hardware and software environment).
Additionally, NTP shall not set large fcntl
...
manager shall store it's logs on (i.e. CLI parameter *-l* shall point to) shared file system, such
as *GPFS* or *GFS2*.
...
which transaction was started, it is crucial that *tmsrv(8)* on different Enduro/X
nodes match following attributes:
...
guarantee transaction log file storage is not used.
...
device. In case if planning to use *tmsrv(8)* *-X* flag for clock skew detection
...
file system. However, *FSYNC* and *-X* parameter may have a performance impact on the
system transaction throughput.

To guarantee that transaction consistency even if *STONITH* is not configured, *tmsrv(8)*
has following features implemented:

- *tmsrv(8)* after the commit decision logged, *tmsrv* check's its singleton group
lock status (which includes the refresh time-stamp verfication). If the group was
not refreshed in the given time *tmsrv(8)* terminates with the failure, as there
might be other node which have picked up the transaction for completion.

- *tmsrv(8)* has implemented additional flag *-X* which verifies the presence of the
transaciton log file (it must exist) and file modification date. If log file
does not exist, or modification date is in future (i.e. )
