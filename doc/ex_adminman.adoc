...
released *lockfile_1* to another node), is *60* seconds (locked_wait). This means that
total time for the survived node to boot would depend on shared file system
re-configuring time-outs/settings, plus this wait time. The wait
time shall be at-least two times higher than other node refresh time, as that
