...
*tmsrv* for performance reason such info logs to disk with *fflush()* Unix call, w
...
==== tmqueue queue space in active-active environment
...
file system (for example *GFS2* or *GPFS*) must be configured for the Enduro/X nodes
to support singleton process groups.
...
used, then checks are done against ping file (to detect the highest lock
counter of all involved nodes). If locked node detects that other nodes
has higher counter value, then given nodes performs unlock and after that
*ndrxd* and *cpmsrv* steps in to kill the group processes.

- For distributed transactions *tmsrv(8)* after commit decsion logged,
verifies that it is still the locked node. If *STONITH* device
is used for the cluster (if it guarantees that fcntl() on new
node does not get the lock unless *STONITH* of broken node has happened),
settings *sg_nodes_verify* can be set be to *N*, so that
for peformance reasons, *tmsrv* on commits / aborts would not check
the cluster lock consistency.
...
recover from cases when parallel instances have generate Qqueue messages or
transactions. *tmsrv* willload un-seen transaction
...
[@exsinglesv/GRP_B]
lockfile_1=/path/to/shared/storage/GRP_B/lock_1
lockfile_2=/path/to/shared/storage/GRP_B/lock_2
...
[@queue/GRP_B]
Q1=svcnm=QYY1,autoq=T,tries=3,waitinit=0,waitretry=1,waitretrymax=5,memonly=n,mode=fifo,workers=5
Q2=svcnm=QYY2,autoq=T,tries=3,waitinit=0,waitretry=1,waitretrymax=5,memonly=n,mode=fifo,workers=5
...
[@global/GRP_B]
NDRX_XA_RES_ID=2
NDRX_XA_OPEN_STR=datadir="/path/to/shared/storage/qsp/BSPACE",qspace="BSPACE"
NDRX_XA_CLOSE_STR=${NDRX_XA_OPEN_STR}
NDRX_XA_DRIVERLIB=libndrxxaqdisks.so
NDRX_XA_RMLIB=libndrxxaqdisk.so
NDRX_XA_LAZY_INIT=0
...
<endurox>
	...
	<procgroups>
		...
		<procgroup grpno="7" name="GRP_B" singleton="Y" sg_nodes="1,2" sg_nodes_verify="Y"/>
	</procgroups>
	<defaults>
	...
	</defaults>
	...
	...
		...
		<server name="tmsrv">
			...
			<sysopt>-e ${NDRX_ULOG}/TM1-a.log -r -- -t60 -l /space/to/shared/storage/tmlogs/rm1 -m10 -n2 -s3 -X6</sysopt>
		</server>
		...
		<server name="tmqueue">
			...
			<cctag>GRP_A</cctag>
			<procgrp>GRP_A</procgrp>
			<sysopt>-e ${NDRX_ULOG}/tmqueue-a.log -r -- -s1</sysopt>
		</server>
		...
		<server name="exsingleckl">
			...
			<max>3</max>
			<srvid>250</srvid>
			<sysopt>-e ${NDRX_ULOG}/exsingleckl-b.log -r</sysopt>
			<procgrp_lp>GRP_B</procgrp_lp>
			<cctag>GRP_B</cctag>
		</server>
		...
		<server name="exsingleckr">
			...
			<max>3</max>
			<srvid>300</srvid>
			<sysopt>-e ${NDRX_ULOG}/exsingleckr-b.log -r</sysopt>
			<procgrp_lp>GRP_B</procgrp_lp>
			<cctag>GRP_B</cctag>
		</server>
		...
		<server name="tmsrv">
			...
			<srvid>350</srvid>
			<cctag>GRP_B</cctag>
			<procgrp>GRP_B</procgrp>
			<!-- shared node id set to 2 (i.e. -n2). Check logs on the disk every 6 seconds    -->
			<sysopt>-e ${NDRX_ULOG}/TM2-b.log -r -- -t60 -l /space/to/shared/storage/tmlogs/rm2 -m10 -s3 -n2 -X6</sysopt>
		</server>
		...
		<server name="tmqueue">
			...
			<cctag>GRP_B</cctag>
			<procgrp>GRP_B</procgrp>
			<!-- Every 60 seconds, check for Q space consistency on the dis.
				 use "2" as virtual node id, so that each Enduro/X instance can regonize
				 the Q files
			-->
			<sysopt>-e ${NDRX_ULOG}/tmqueue-b.log -r -- -s1 -n2 -X60</sysopt>
		</server>

		<!-- run transaction recovery after the tmqueue activation.
		     For other resources may put the tmrecoversv right
			 after the tmsrv.
		 -->
		<server name="tmrecoversv">
			<min>1</min>
		...
		<server name="banksv_b">
			<min>1</min>
			<max>1</max>
			<srvid>420</srvid>
			<procgrp>GRP_A</procgrp>
			<sysopt>-e ${NDRX_ULOG}/banksv-b.log</sysopt>
		</server>

		... other GRP_A servers here ...

		<!-- establish link bweteen nodes 1 & 2 (so transaction
			 checks run faster)
	     -->
	...
	<clients>
        <client cmdline="restincl" cctag="GRP_A" procgrp="GRP_A">
