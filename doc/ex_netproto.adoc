...
|0x11DB|callseq|UINT1..10|Mand|N/A|Any|Call sequence number (is incremented by calling process
for each of the new requests, overlapping). Field is used for request and reply matching. I.e. in
...
Value must be in range of 0..4294967295.
|0x11DC|msgseq|UINT1..10|Mand|N/A|4,5,6,7|Message sequence number for conversational data. As tpbridge
...
|0x12F3|callseq|UINT1..10|Mand|0|Any|RFU. *0* shall be used.
|0x12FD|msgseq|UINT1..10|Mand|0|Any|RFU. *0* shall be used.
