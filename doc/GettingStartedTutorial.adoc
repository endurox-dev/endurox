...
To call the service just pipe the data `ud':
---------------------------------------------------------------------
...
T_ACCCUR	EUR
---------------------------------------------------------------------
We see that our "dummy" balance returned is "1355808545.118969". So test service is working ok. Now we should write a client app, which could call the service via tpcall() XATMI command.
