                        ...
                        long rcv_len, char **odata, long *olen, long flags)
{
    ...
    if (NULL!=outbufobj)
    {
        ...
        if (outbufobj->type_id!=BUF_TYPE_STRING)
        {
            NDRX_LOG(log_info, "User buffer %s is different, "
                    "free it up and re-allocate as STRING", G_buf_descr[outbufobj->type_id].type);
    ...
    else
    {
        ...
        *odata = ndrx_tpalloc(&G_buf_descr[BUF_TYPE_STRING], NULL, NULL, rcv_len);
 ...
 * @return
 */
expublic char * STRING_tpalloc (typed_buffer_descr_t *descr, const char *subtype, long *len)
{
    char *ret;
