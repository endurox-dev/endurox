        ...
        long ilen, char *obuf, long *olen, long flags)
{
     ...
     * Should'n  we reject on 0 - ok?
     */
    if (NULL!=olen && *olen < ubf_used)
    {
                        ...
                        long rcv_len, char **odata, long *olen, long flags)
{
    ...
    if (NULL!=outbufobj)
    {
        ...
        {
            NDRX_LOG(log_info, "User buffer type %s is different, "
                    "free it up and re-allocate as UBF", G_buf_descr[outbufobj->type_id].type);
