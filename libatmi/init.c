...
public int _tpterm (void)
{
    ...
    {
        if (FAIL==mq_close(G_atmi_conf.reply_q))
        {
    ...
    if (EOS!=G_atmi_conf.reply_q_str[0])
    {
        ...
        if (FAIL==mq_unlink(G_atmi_conf.reply_q_str))
        {
...
public int tp_internal_init_upd_replyq(mqd_t reply_q, char *reply_q_str)
{
    ...
    if (FAIL==mq_getattr(reply_q, &G_atmi_conf.q_attr))
    {
...
public int tp_internal_init(atmi_lib_conf_t *init_data)
{
    ...
    if (init_data->reply_q && FAIL==mq_getattr(init_data->reply_q, &G_atmi_conf.q_attr))
    {
