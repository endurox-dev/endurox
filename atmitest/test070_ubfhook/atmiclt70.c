...
int main(int argc, char** argv)
{
    ...
    if (EXFAIL==CBchg(p_ub, T_STRING_6_FLD, 0, "HELLO WORLD", 0, BFLD_STRING))
    {
        NDRX_LOG(log_debug, "Failed to set T_STRING_6_FLD[0]: %s", Bstrerror(Berror));
        ret=EXFAIL;
        goto out;
    }
    
    if (EXFAIL==CBchg(p_ub, T_STRING_7_FLD, 0, "WORLD", 0, BFLD_STRING))
    {
        NDRX_LOG(log_debug, "Failed to set T_STRING_7_FLD[0]: %s", Bstrerror(Berror));
        ret=EXFAIL;
        goto out;
    }
