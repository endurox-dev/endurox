/**
 * @brief Mask service in server config (no advertise by -n) - server
 *
 * @file atmisv49.c
 */
/* -----------------------------------------------------------------------------
...
int NDRX_INTEGRA(tpsvrinit)(int argc, char **argv)
{
    ...
    if (EXSUCCEED!=tpadvertise("TESTSV0", TESTSV))
    {
        NDRX_LOG(log_error, "Failed to initialize TESTSV0!");
    }
    ...
    if (EXSUCCEED!=tpadvertise("TESTSV2", TESTSV))
    {
        NDRX_LOG(log_error, "Failed to initialize TESTSV2!");
    }
    
    if (EXSUCCEED!=tpadvertise("TESTSV3", TESTSV))
    {
        NDRX_LOG(log_error, "Failed to initialize TESTSV3!");
    }
    
    if (EXSUCCEED!=tpadvertise("TESTSV4", TESTSV))
    {
        NDRX_LOG(log_error, "Failed to initialize TESTSV4!");
    }
