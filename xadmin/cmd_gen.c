...
extern const char ndrx_G_resource_gen_python_server[];
extern const size_t ndrx_G_resource_gen_python_server_len;
...
exprivate int reg_cmd(char *command, const char *stock, size_t stock_len, const char *fname)
{
    ...
    NDRX_STRCPY_SAFE(gen->command, command);
...
#define EMB_RES(name) name, name##_len
    ...
    if (EXSUCCEED!=reg_cmd("go server", EMB_RES(ndrx_G_resource_gen_go_server), NULL)
        ...
        || EXSUCCEED!=reg_cmd("python server", EMB_RES(ndrx_G_resource_gen_python_server), NULL)
    ...
    if (NULL!=(val=ndrx_keyval_hash_get(G_xadmin_config, "gen scripts")))
    {
        if (NULL!=(flist = ndrx_sys_folder_list(val->val, &return_status)))
        {
            ...
            LL_FOREACH(flist,elt)
            {
                ...
                {
                    NDRX_STRCPY_SAFE(tmp, elt->qname);
