 ...
 * @param p_ub UBF buffer
 */
expublic void   ndrx_tplogprintubf(int lev, const char *title, UBFH *p_ub)
{
    API_ENTRY;
...
#define NDRX_UBF_GET_FLDIDOCC(LAST_ARG) \
    ...
    {\
        ...
        {\
            ndrx_Bset_error_fmt(BBADFLD, "FLDID,OCC path too long, max %d", \
                    BFLDOCCMAX);\
            va_end(ap);\
            EXFAIL_OUT(ret);\
        }\
    ...
    if ( pos % 2 == 0)\
    {\
        ndrx_Bset_error_fmt(BBADFLD, "Expected odd number FLDID,OCC,..,"\
    ...
    if (1==pos)\
    {\
        ndrx_Bset_error_fmt(BBADFLD, "Expected FLDID,OCC,..,"\
