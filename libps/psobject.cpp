...
bool PSGenerator::Yield(PSVM *v,PSInteger target)
{
    ...
    for(PSInteger i=0;i<_ci._etraps;i++) {
        ...
        PSExceptionTrap &et = _etraps.back();
...
bool PSGenerator::Resume(PSVM *v,PSObjectPtr &dest)
{
    ...
    for(PSInteger i=0;i<_ci._etraps;i++) {
        ...
        _etraps.pop_back();
        PSExceptionTrap &et = v->_etraps.back();
...
bool WriteObject(HPSCRIPTVM v,PSUserPointer up,PSWRITEFUNC write,PSObjectPtr &o)
{
    ...
    case OT_STRING:
        ...
        _CHECK_IO(SafeWrite(v,write,up,_stringval(o),ps_rsl(_string(o)->_len)));
        break;
    case OT_BOOL:
    case OT_INTEGER:
...
bool ReadObject(HPSCRIPTVM v,PSUserPointer up,PSREADFUNC read,PSObjectPtr &o)
{
    ...
    case OT_STRING:{
        ...
        _CHECK_IO(SafeRead(v,read,up,_ss(v)->GetScratchPad(ps_rsl(len)),ps_rsl(len)));
...
bool PSClosure::Load(PSVM *v,PSUserPointer up,PSREADFUNC read,PSObjectPtr &ret)
{
    ...
    ret = PSClosure::Create(_ss(v),_funcproto(func),_table(v->_roottable)->GetWeakRef(OT_TABLE));
