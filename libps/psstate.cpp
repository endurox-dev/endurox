...
PSTable *CreateDefaultDelegate(PSSharedState *ss,PSRegFunction *funcz)
{
	PSInteger i=0;
...
void PSSharedState::RunMark(PSVM *vm,PSCollectable **tchain)
{
...
PSString *PSStringTable::Add(const PSChar *news,PSInteger len)
{
	...
	PSString *t = (PSString *)PS_MALLOC(rsl(len)+sizeof(PSString));
	new (t) PSString;
...
void PSStringTable::Remove(PSString *bs)
{
	...
	for (s = _strings[h]; s; ){
		if(s == bs){
			...
			s->~PSString();
			PS_FREE(s,sizeof(PSString) + rsl(slen));
			return;
		}
		prev = s;
		s = s->_next;
