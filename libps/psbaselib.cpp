...
bool str2num(const PSChar *s,PSObjectPtr &res)
{
	PSChar *end;
	const PSChar *e = s;
	PSBool isfloat = PSFalse;
	PSChar c;
	while((c = *e) != _SC('\0'))
	{
	...
	if(isfloat){
		...
		PSInteger r = PSInteger(scstrtol(s,&end,10));
		if(s == end) return false;
		res = r;
	}
	return true;
...
static PSInteger base_dummy(HPSCRIPTVM v)
{
	return 0;
}
...
static PSRegFunction base_funcs[]={
	//generic
...
PSRegFunction PSSharedState::_table_default_delegate_funcz[]={
...
bool _sort_compare(HPSCRIPTVM v,PSObjectPtr &a,PSObjectPtr &b,PSInteger func,PSInteger &ret)
{
	if(func < 0) {
...
bool _hsort_sift_down(HPSCRIPTVM v,PSArray *arr, PSInteger root, PSInteger bottom, PSInteger func)
{
	PSInteger maxChild;
	PSInteger done = 0;
	PSInteger ret;
...
bool _hsort(HPSCRIPTVM v,PSObjectPtr &arr, PSInteger l, PSInteger r,PSInteger func)
{
...
static PSInteger array_slice(HPSCRIPTVM v)
{
	...
	if(eidx > alen)return ps_throwerror(v,_SC("slice out of range"));
...
PSRegFunction PSSharedState::_array_default_delegate_funcz[]={
...
static PSInteger string_slice(HPSCRIPTVM v)
{
	...
	if(eidx < sidx)	return ps_throwerror(v,_SC("wrong indexes"));
	if(eidx > slen)	return ps_throwerror(v,_SC("slice out of range"));
...
{ \
	PSObject str=stack_get(v,1); \
	PSInteger len=_string(str)->_len; \
	const PSChar *sThis=_stringval(str); \
	PSChar *sNew=(_ss(v)->GetScratchPad(rsl(len))); \
	for(PSInteger i=0;i<len;i++) sNew[i]=func(sThis[i]); \
...
PSRegFunction PSSharedState::_string_default_delegate_funcz[]={
...
PSRegFunction PSSharedState::_number_default_delegate_funcz[]={
...
PSRegFunction PSSharedState::_closure_default_delegate_funcz[]={
...
PSRegFunction PSSharedState::_generator_default_delegate_funcz[]={
...
static PSInteger thread_wakeup(HPSCRIPTVM v)
{
	...
	if(type(o) == OT_THREAD) {
		...
		PSInteger wakeupret = ps_gettop(v)>1?1:0;
		if(wakeupret) {
			ps_move(thread,v,2);
		}
...
PSRegFunction PSSharedState::_thread_default_delegate_funcz[] = {
...
PSRegFunction PSSharedState::_class_default_delegate_funcz[] = {
...
PSRegFunction PSSharedState::_instance_default_delegate_funcz[] = {
...
PSRegFunction PSSharedState::_weakref_default_delegate_funcz[] = {
