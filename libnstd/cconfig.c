 ...
 * @return 
 */
expublic int ndrx_cconfig_get_cf(ndrx_inicfg_t *cfg, const char *section, 
...
exprivate int ndrx_apiflags_load(void)
{
    ...
    if (NULL!=p)
    {
        ...
        DL_FOREACH(parsed, el)
        {
            ...
            else if (0==strcmp(el->key, NDRX_APIFLAGS_UBFDUPFIDOK_CODE))
            {
                NDRX_LOG_EARLY(log_info, "%s UBF table duplicate field id OK",
